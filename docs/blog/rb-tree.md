...
如果有这样的序列 s1,s2,s3...,`arr[i]`表示的就是 `s[i-lowbit(i) + 1] + s[i-lowbit(i) + 1 + 1] + ... + s[i]`。
...
如果你想计算 `s[3]`, 只需要像上边的计算方式，先计算`s[1]+s[2]+s[3]`,再计算`s[1]+s[2]`，然后前者减去后者即可。
...
二叉树是拥有左右两个分支的树，B Tree 是拥有多个分支的树。m 阶 B Tree 意味着：

- 每个节点最多有 m 个子节点，最少有 ceil(m/2) 个子节点
- 树根节点可以有最少 2 个子节点
- 要存储的数据分布在 B Tree 的内部节点和叶子节点

3 阶 B Tree(最多 2 个 key，最少 1 个 key)， 插入操作演示
...
B+ Tree 拥有 B Tree 的所有特征，只有下面两点不同：

1. 要存储的数据全部分布在叶子节点
2. 全部叶子节点使用 prev next 指针串连成一条链表

## B\* Tree

B\* Tree 在 B+ Tree 的基础上，做了如下的调整：

1. 每一层内部节点也使用 prev next 指针串连成一条链表
2. 每个节点最少有 ceil(m/3) 个子节点
