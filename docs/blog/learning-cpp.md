...
### cpp 哪里优于 Rust，哪里不如 Rust
cpp的最大优势在于语法兼容c，可以不用借助FFI，直接使用c代码，Rust做不到。

另外一个优势在于引入class，让c变得更结构化。而且，随之引入的RAII，改进了c申请和释放。但它的这种改进存在局限，因为它只是优化了内存申请和释放的触发方式，不再由程序员手动触发，而是编译器生成对应的触发代码。而申请内存、释放内存，要执行的内容，依旧依赖程序员。如果这两个环节，牵扯到指针操作，就会退化到c语言的情形。

但cpp的问题，在c style的cpp代码中，不是很严重。糟糕之处在于，cpp引入移动、拷贝、引用的概念。c之所以不安全，就是因为指针的操作，都依赖程序员保证安全。这部分指针的问题，cpp没有改进，而且，cpp新增的引用，其是否有效，也依赖程序员保证，随着移动构造、拷贝构造的机制，这种问题会被进一步放大。这方面，Rust就赢麻了。得到的结论就是，使用现代c++编程，真得不如用Rust，cpp的价值点还是在于 c style的代码，不是其现代特性，它的现代特性解决了一部分安全问题，也带来了新的安全问题和学习负担。相比之下，Rust解决了安全问题，但只带来了学习成本的问题，而不是新的安全问题。
